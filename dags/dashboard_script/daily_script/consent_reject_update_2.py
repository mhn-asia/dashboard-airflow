...
def get_consent_reject_update_2(**kwargs):
    ...
    if not consentData:  # Check if there is no data
        ...
        consent_df['meta.lastUpdated'] = pd.to_datetime(consent_df['meta.lastUpdated'], utc=True)
        consent_df['year'] = consent_df['meta.lastUpdated'].dt.year
        consent_df['month'] = consent_df['meta.lastUpdated'].dt.month
        consent_df = consent_df.drop('meta.lastUpdated', axis=1)

        consent_new_df = pd.DataFrame({
            'year' : consent_df['year'],
            'month' : consent_df['month']
        })
        ...
        table_name = "consent_reject_summary_2"
        conn = connection
        cursor = conn.cursor()
        ...
        try:
            for _, row in consent_reject_summary_df.iterrows():
                insert_query = f"""
                INSERT INTO {table_name} (year, month, reject_count)
                VALUES (%s, %s, %s);
                """
                values = (
                    int(row['year']),
            ...
            telegram_msg(msg='Done update consent_reject_2 for date '+yesterday_string, conf=TOKEN_BOT_CHANNEL)
